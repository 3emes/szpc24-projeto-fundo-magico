document.addEventListener("DOMContentLoaded", function () {
	...
	formulario.addEventListener("submit", async function (evento) {
		...
		try {
			const resposta = await fetch("https://matheusm123.app.n8n.cloud/webhook/projeto fundo magico", {
				method: "POST",
				headers: {
...
// Caso o backend ainda retorne tudo dentro de "output"
if (respostaJson.output && typeof respostaJson.output === "string") {
	let textoLimpo = respostaJson.output
		.replace(/```json/g, "")
		.replace(/```/g, "")
		.trim();
